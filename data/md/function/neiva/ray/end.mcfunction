...
execute unless score #noTP md.rayOutput matches 1 if entity @e[tag=md.this,type=#arrows] run tellraw @a[tag=md.dev] "Ray: Granting 1 blocks of clearance for arrow projectile"
execute unless score #noTP md.rayOutput matches 1 if entity @e[tag=md.this,type=firework_rocket] run tp ^ ^ ^-15
execute unless score #noTP md.rayOutput matches 1 if entity @e[tag=md.this,type=firework_rocket] run tellraw @a[tag=md.dev] "Ray: Granting 15 blocks of clearance for firework projectile"
